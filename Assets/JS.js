...
function init(e) {
    ...
    if (checkCity(locationRequest)) {
        savedCities();
        createSearchResults(locationRequest);
    }
    ...
    fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${locationRequest}&limit=1&appid=${APIkey}`)
...
function createSearchResults(location) {
    ...
    //if condition, to see if there are items inside array before attempting to populate search list
    if (cities !== null) {
        for (let index = 0; index < cities.length; index++) {
            historyResult.innerHTML = cities[index];
            listofSearch.append(historyResult);
        }
        //if there is no items inside cities array, then population search history with just current search query
    } else if (cities == null) {
        historyResult.innerHTML = location;
        listofSearch.append(historyResult);
...
//creates list of search history when loading up the page
function startingSearchResults() {
    cities = JSON.parse(localStorage.getItem("Searched Cities"));
    //if there are items inside cities array, then add them to search history
    if (cities !== null) {
        for (let index = 0; index < cities.length; index++) {
            var historyResult = document.createElement("div");
            historyResult.innerHTML = cities[index];
        ...
        //if there are no items in cities array, then exit function
    } else if (cities == null) {
        //changes cities array back to empty array instead of null
        cities = [];
        return;
    }
}
...
window.onload = startingSearchResults()